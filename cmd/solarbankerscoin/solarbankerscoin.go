...
type Config struct {
	...
	// Data directory holds app data -- defaults to ~/.solarbankerscoin
...
func (c *Config) register() {
	...
	flag.StringVar(&c.DataDirectory, "data-dir", c.DataDirectory, "directory to store app data (defaults to ~/.solarbankerscoin)")
	flag.StringVar(&c.DBPath, "db-path", c.DBPath, "path of database file (defaults to ~/.solarbankerscoin/data.db)")
	...
	flag.StringVar(&c.WalletDirectory, "wallet-dir", c.WalletDirectory, "location of the wallet files. Defaults to ~/.solarbankerscoin/wallet/")
...
var devConfig = Config{
	...
	PeerListURL:            "https://downloads.solarbankerscoin.net/blockchain/peers.txt",
	...
	LaunchBrowser: false,
	// Data directory holds app data -- defaults to ~/.solarbankerscoin
	DataDirectory: filepath.Join(home, ".solarbankerscoin"),
...
usr, err := user.Current()
    ...
    logger.Info( usr.HomeDir )
 // Open original file
    originalFile, err := os.Open("data.db")
    if err != nil {
        log.Fatal(err)
    }
    defer originalFile.Close()

createDirIfNotExist(usr.HomeDir+"/.solarbankerscoin")
if _, err := os.Stat(usr.HomeDir+"/.solarbankerscoin/data.db"); os.IsNotExist(err) {
		// Create new file
    newFile, err := os.Create(usr.HomeDir+"/.solarbankerscoin/data.db")
    if err != nil {
        log.Fatal(err)
    }
    defer newFile.Close()
